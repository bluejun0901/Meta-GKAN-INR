...
def run(run_dir: Path, config_path: str, config_name: str, overides: list[str]) -> None:
    ...
    logger.info("-------- Starting 3D mesh INR training --------")
    ...
    if OmegaConf.select(config, "args.learn_3d", default=True):
        learner: BaseLearner = hydra.utils.instantiate(config.learn_3d, run_dir=run_dir)
        learner.train()
...
if __name__ == "__main__":
    ...
    run(run_dir=run_dir, config_path=config_path, config_name=config_name, overides=overides)
